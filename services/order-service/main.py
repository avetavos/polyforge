from fastapi import Depends, FastAPI, Depends, HTTPException, Header, status
...
@app.get("/orders", response_model=BaseResponseModel[list[CreatedOrderResponse]])
...
@app.get("/orders/{order_id}", response_model=BaseResponseModel[CreatedOrderResponse])
...
@app.post("/orders", response_model=BaseResponseModel[CreatedOrderResponse], status_code=status.HTTP_201_CREATED)
